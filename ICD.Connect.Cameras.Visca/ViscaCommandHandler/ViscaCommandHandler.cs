        ...
        public string Move(int Id, eCameraAction action, int panSpeed, int tiltSpeed, int zoomSpeed)
        {
            switch (action)
            {
                case eCameraAction.Up:
                    return StringUtils.ToString(new byte[] { GetIdsByte(0, Id), 0x01, 0x06, 0x01, GetPanSpeedByte(panSpeed), GetTiltSpeed(tiltSpeed), 0x03, 0x01, 0xFF });
                case eCameraAction.Down:
                    return StringUtils.ToString(new byte[] { GetIdsByte(0, Id), 0x01, 0x06, 0x01, GetPanSpeedByte(panSpeed), GetTiltSpeed(tiltSpeed), 0x03, 0x02, 0xFF });
                case eCameraAction.Left:
                    return StringUtils.ToString(new byte[] { GetIdsByte(0, Id), 0x01, 0x06, 0x01, GetPanSpeedByte(panSpeed), GetTiltSpeed(tiltSpeed), 0x01, 0x03, 0xFF });
                case eCameraAction.Right:
                    return StringUtils.ToString(new byte[] { GetIdsByte(0, Id), 0x01, 0x06, 0x01, GetPanSpeedByte(panSpeed), GetTiltSpeed(tiltSpeed), 0x02, 0x03, 0xFF });
