...
{
	public sealed class AecInputBlock : AbstractIoBlock
	{
		...
		private readonly Dictionary<int, AecInputChannel> m_Channels;
		...
		public AecInputBlock(BiampTesiraDevice device, string instanceTag)
			: base(device, instanceTag)
		{
			m_Channels = new Dictionary<int, AecInputChannel>();
		...
		/// <returns></returns>
		[PublicAPI]
		public IEnumerable<AecInputChannel> GetChannels()
		{
		...
		private void RebuildChannels()
		{
			...
			try
			{
				Enumerable.Range(1, ChannelCount).ForEach(i => LazyLoadInputChannel(i));
			}
			finally
			{
		...
		/// <returns></returns>
		private AecInputChannel LazyLoadInputChannel(int index)
		{
			m_ChannelsSection.Enter();
			...
			try
			{
				if (!m_Channels.ContainsKey(index))
					m_Channels[index] = new AecInputChannel(this, index);
