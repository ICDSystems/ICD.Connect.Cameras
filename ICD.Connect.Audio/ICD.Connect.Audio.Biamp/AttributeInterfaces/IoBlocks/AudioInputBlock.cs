...
{
	public sealed class AudioInputBlock : AbstractIoBlock
	{
		...
		private readonly Dictionary<int, AudioInputChannel> m_Channels;
		...
		public AudioInputBlock(BiampTesiraDevice device, string instanceTag)
			: base(device, instanceTag)
		{
			m_Channels = new Dictionary<int, AudioInputChannel>();
		...
		/// <returns></returns>
		[PublicAPI]
		public IEnumerable<AudioInputChannel> GetChannels()
		{
		...
		/// <returns></returns>
		private AudioInputChannel LazyLoadChannel(int index)
		{
			m_ChannelsSection.Enter();
			...
			try
			{
				if (!m_Channels.ContainsKey(index))
					m_Channels[index] = new AudioInputChannel(this, index);
