...
{
	public sealed class SettingsDevicePropertiesPortComponentPresenter :
		AbstractSettingsDevicePropertiesListComponentPresenter,
		ISettingsDevicePropertiesPortComponentPresenter
	{
		/// <summary>
		/// Constructor.
		/// </summary>
		...
		public SettingsDevicePropertiesPortComponentPresenter(int room, INavigationController nav,
		...
		protected override IEnumerable<object> GetListValues()
		{
			...
			return coreSettings.OriginatorSettings
			                   .Where(s => s.OriginatorType.IsAssignableTo(typeof(IPort)))
			                   .Select(p => p.Id)
			                   .Cast<int?>()
			                   .Order()
			                   .Prepend(null)
