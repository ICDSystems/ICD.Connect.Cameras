...
namespace ICD.Connect.Partitions
{
	public abstract class AbstractPartitionSettings : AbstractSettings, IPartitionSettings
	{
		...
		{
			if (PartitionDevice.HasValue)
				yield return PartitionDevice.Value;
		}
		...
		protected override void WriteElements(IcdXmlTextWriter writer)
		{
			...
			writer.WriteElementString(PARTITION_DEVICE_ELEMENT, IcdXmlConvert.ToString(PartitionDevice));
		...
		protected static void ParseXml(AbstractPartitionSettings instance, string xml)
		{
			...
			instance.PartitionDevice = XmlUtils.TryReadChildElementContentAsInt(xml, PARTITION_DEVICE_ELEMENT);
